class Game {
  ...
  play() {
    ...
    if (allPlayers !== undefined) {
      ...
      var x = 175;
      var y = 100;
    //  reading values for snow ball from the database;
        ...
        if (index === player.index) {
          ...
          if (keyDown("space")) {
            ...
            if (index == 3 && index == 4) {
              ...
              // database.ref("snowBall/position").on("value",(data)=>{
              //   var readValue = data.val();
  ...
  createSnow = (x, y, dir) => {
    state = 1
    player.snowSprite = createSprite(x, y)
    player.update()
    database.ref("snowBall/position").update({
      x: player.snowSprite.x, y: player.snowSprite.y
    
    })
    player.snowSprite.lifeTime = 300;
    player.snowSprite.addImage(snow_img)
    player.snowSprite.scale = 0.07
    if (dir == 0) {
      player.snowSprite.x -= 8;
    } else if (dir == 1) {
      player.snowSprite.x += 8;
    }
